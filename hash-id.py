...
def get_hash(hashimp):
    ...
    try:
        ...
        if len(jerar)==0:
            print("\n Not Found.")
            return("Null") # No hash to return
        elif len(jerar)>2:
            jerar.sort()
            
            returnhash = [str(algorithms[jerar[0]]), str(algorithms[jerar[1]])]
            #print("\nLeast Possible Hashs:")
            for a in range(int(len(jerar))-2):
                #print("[+] "+str(algorithms[jerar[a+2]]))
                returnhash.append(str(algorithms[jerar[a+2]]))
            ...
            for a in range(len(jerar)):
                ...
                returnhash[a] = str(algorithms[jerar[a]])
        return(returnhash)
        first = None
